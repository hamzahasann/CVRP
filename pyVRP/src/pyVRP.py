...
# GA-VRP Function
def genetic_algorithm_vrp(solution_report,solution,coordinates, distance_matrix, parameters, velocity, fixed_cost, variable_cost, capacity, population_size = 5, vehicle_types = 1, n_depots = 1, route = 'closed', model = 'vrp', time_window = 'without', fleet_size = [], mutation_rate = 0.1, elite = 0, generations = 50, penalty_value = 1000, graph = True, selection = 'rw'):    
    start           = tm.time()
    count           = 0
    ...
    solution.append(copy.deepcopy(population[0]))
    solution=copy.deepcopy(solution[0])
    yield('Generation = {}, Distance = {}, f(x) = {}'.format(count,elite_ind,round(elite_cst,2)))
    ...
    yield('Algorithm Time: {} seconds'.format(round(end-start,2)))
